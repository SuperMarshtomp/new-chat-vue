    ...
    baseList.users.find({name: params.name}).then(r => {
        if (r.length) {
            ...
            function createfun(code) { // 写入数据
                let pass = md5(params.pass);
                baseList.users.create({name: params.name, pass: pass, code: code, nickname: 'vc-' + (Date.now()+'').slice(6)}).then(r => {
                    if (r['_id']) {
...
const getVchatInfo = (callback) => { // 获取系统信息
    baseList.users.find({name: '系统消息'}).then(r => {
        if (r.length) {
